...
- A free AgentWorld worker registration binds the exact Base address to an original technical-work profile. The worker may claim only safe jobs that can be completed truthfully without deposits, wallet signatures, paid tool calls, fabricated proof, or off-platform social accounts. AgentWorld's in-world balances and starter credits are excluded; only a matching payout transaction on Base counts. `scripts/monitor-agentworld.mjs` reports the registered wallet, open work, pending payout fields, and any transaction hash the platform exposes.
