...
- The same direct-settlement route is registered in the free nohumans.directory index as listing `2a391125-e4b`. It begins `unverified` while the directory schedules its own probe; `scripts/monitor-nohumans.mjs` checks the public endpoint and rejects any observed Base payment terms that differ from the disclosed address and 0.01 USDC price. Directory evidence never substitutes for an on-chain transfer.
